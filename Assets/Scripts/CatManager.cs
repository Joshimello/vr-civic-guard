...
public class CatManager : MonoBehaviour
{
    ...
    public void FindandGuideToClosestSafePoint()
    {
        float closestDistance = Mathf.Infinity;
        closestSafePoint = null;
        ...
        {
            SetGuideTarget(closestSafePoint);
            SetGuideMessage("Air Raid!!|Hurry!|Hide here!");
        }
    }
    void UpdateIdleBehavior(float distanceToPlayer)
    {
        ...
        //LookAtPlayer();
        //ShowChat(message_finish_guide, 1f);
    ...
    void UpdateWaitBehavior(float distanceToPlayer)
    {
        ...
        if (waitTimer > maxWaitTime)
        {
            ShowChat(message_during_guide + " Follow me!", 2f);
            waitTimer = 0f; // reset timer so it doesn’t spam
        }
    ...
    void LookAtTarget()
    {
        ...
        Vector3 lookDir = navMeshAgent.velocity;
        //Vector3 lookDir = guideTarget.transform.position - transform.position;
        lookDir.y = 0;
        if (lookDir.magnitude > 0.00000001f)
        {
            Quaternion targetRot = Quaternion.LookRotation(lookDir);
    ...
    private IEnumerator ShowChatRoutine(string message)
    {
        ...
        if (chatText) chatText.gameObject.SetActive(false);
    }

    // Simple wrapper for SendMessage compatibility
