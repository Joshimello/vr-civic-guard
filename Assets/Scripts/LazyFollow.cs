...
namespace UnityEngine.XR.Interaction.Toolkit.UI
{
    ...
    public class LazyFollow : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("(Optional) The object being followed for position. If not set, this will default to the main camera when this component is enabled.")]
        Transform m_Target;

        /// <summary>
        /// The object being followed for position. If not set, this will default to the main camera when this component is enabled.
        /// </summary>
        ...
        [SerializeField, Tooltip("(Optional) The object to look at for rotation. If not set, this will use the same target as the follow target.")]
        Transform m_LookAtTarget;

        /// <summary>
        /// The object to look at for rotation. If not set, this will use the same target as the follow target.
        /// </summary>
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        protected void OnEnable()
        {
            ...
            // Default look at target to the follow target if not explicitly set
            if (m_LookAtTarget == null)
            {
                m_LookAtTarget = m_Target;
            }
        ...
        {
            if (m_Target == null && m_LookAtTarget == null)
                return;

            var deltaTime = Time.unscaledDeltaTime;

            if (m_PositionFollowMode != PositionFollowMode.None && m_Target != null)
            {
                if (TryGetThresholdTargetPosition(out var newPositionTarget))
                    m_Vector3TweenableVariable.target = newPositionTarget;
        ...
        protected virtual bool TryGetThresholdTargetRotation(out Quaternion newTarget)
        {
            switch (m_RotationFollowMode)
            {
                ...
                case RotationFollowMode.LookAt:
                    {
                        var lookTarget = m_LookAtTarget != null ? m_LookAtTarget : m_Target;
                        if (lookTarget == null)
                        {
                            newTarget = followInLocalSpace ? transform.localRotation : transform.rotation;
                            return false;
                        }
                        var forward = (transform.position - lookTarget.position).normalized;
                ...
                case RotationFollowMode.Follow:
                    var followTarget = m_LookAtTarget != null ? m_LookAtTarget : m_Target;
                    if (followTarget == null)
                    {
                        newTarget = followInLocalSpace ? transform.localRotation : transform.rotation;
                        return false;
                    }
                    newTarget = followInLocalSpace ? followTarget.localRotation : followTarget.rotation;
                    break;
